    ...
    */
    ...
    for (i=0; i<m; i++){
        ...
        for (j=0; j<n; j+=4) {
            ...
            // Reducimos, sin usar hadd
            // [1 2 3 4]  [3 4 2 1] <- shuffle
            //          \/ add
            //   [1+3 2+4 3+2 4+1]
            //   [ 4   6   ~   ~]
            ...
            temp_v = _mm_shuffle_ps(y_i, y_i, _MM_SHUFFLE(0,1,3,2));
            y_i = _mm_add_ps(y_i, temp_v);

            temp_v = _mm_shuffle_ps(y_i, y_i, _MM_SHUFFLE(3,2,0,1));
            y_i = _mm_add_ps(y_i, temp_v);

            _mm_store_ps(temp, y_i);
            y[i] += temp[0];
